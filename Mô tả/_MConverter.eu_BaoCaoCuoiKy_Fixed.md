...
**PHẦN MỀM QUẢN LÝ CƯ DÂN -- ResidentIQ**
...
# CHƯƠNG 1. PHÂN TÍCH YÊU CẦU (Tuần 1--3) {#chương-1.-phân-tích-yêu-cầu-tuần-13}

Giai đoạn này xác định toàn bộ yêu cầu cho hệ thống. Vấn đề cốt lõi: ban quản lý chung cư/nhà trọ dùng sổ sách giấy dẫn đến khó tra cứu, dễ sai sót và không theo dõi được tạm trú/tạm vắng theo thời gian thực.

## 1.1 Người dùng hệ thống {#người-dùng-hệ-thống}

|  |  |  |
|----|----|----|
| **Người dùng** | **Role** | **Quyền hạn** |
...
## 1.2 Yêu cầu chức năng & phi chức năng {#yêu-cầu-chức-năng-phi-chức-năng}
...
# CHƯƠNG 2. KIẾN TRÚC HỆ THỐNG (Tuần 4) {#chương-2.-kiến-trúc-hệ-thống-tuần-4}

Kiến trúc được chọn: Modular Monolith -- một server duy nhất, phân chia rõ theo module chức năng. Phù hợp team 3 người, deadline ngắn, tránh overhead của Microservices.
...
## 2.2 Cấu trúc thư mục Backend {#cấu-trúc-thư-mục-backend}
...
# CHƯƠNG 3. THIẾT KẾ CẤP CAO (Tuần 5) {#chương-3.-thiết-kế-cấp-cao-tuần-5}

## 3.1 Thiết kế API -- RESTful Endpoints {#thiết-kế-api-restful-endpoints}
...
## 3.2 Thiết kế MongoDB Collections {#thiết-kế-mongodb-collections}

|  |  |  |
|----|----|----|
| **Collection** | **Trường dữ liệu chính** | **Ràng buộc** |
...
## 3.3 Thiết kế Bảo mật {#thiết-kế-bảo-mật}
...
# CHƯƠNG 4. THIẾT KẾ CẤP THẤP (Tuần 6) {#chương-4.-thiết-kế-cấp-thấp-tuần-6}

## 4.1 Sơ đồ Lớp (Class Diagram) {#sơ-đồ-lớp-class-diagram}

|  |  |  |
|----|----|----|
| **Lớp** | **Phương thức** | **Phụ thuộc** |
...
## 4.2 Sơ đồ Trình tự -- Đăng nhập {#sơ-đồ-trình-tự-đăng-nhập}

1.  1\. Client → POST /api/auth/login {usernameOrEmail, password}

2.  2\. login(): kiểm tra body đủ trường → thiếu trả 400.

3.  3\. User.findOne({\$or:\[{username},{email}\]}) tìm trong MongoDB.

4.  4\. Không tìm thấy → 401 \"Thông tin đăng nhập không chính xác\".

5.  5\. bcrypt.compare(password, user.passwordHash) so sánh.

6.  6\. Sai mật khẩu → 401.

7.  7\. Đúng → jwt.sign({id,role,username}, JWT_SECRET, {expiresIn:\"7d\"}).

8.  8\. Trả 200 {token, user: sanitizeUser(user)} (đã loại bỏ passwordHash).

## 4.3 Sơ đồ Trình tự -- Thêm cư dân & Tạo tài khoản {#sơ-đồ-trình-tự-thêm-cư-dân-tạo-tài-khoản}

9.  1\. Client → POST /api/residents {name, cccd, dob, gender, room, regdate, \...}

10. 2\. requireAuth: jwt.verify() + User.findById() → lỗi trả 401.

11. 3\. requireRole(\"admin\",\"staff\"): role không khớp → trả 403.

12. 4\. Kiểm tra trường bắt buộc (cccd, name, dob, gender, room, regdate) → thiếu trả 400.

13. 5\. Resident.findOne({cccd}) → CCCD đã tồn tại → trả 409 Conflict.

14. 6\. Resident.create({\...data, status:\"Thường trú\", history:\[{action:\"Thêm mới cư dân\"}\]}).

15. 7\. defaultPassword = cccd.slice(-8) (8 số cuối CCCD).

16. 8\. bcrypt.hash(defaultPassword, 10) mã hóa.

17. 9\. User.create({username:cccd, email:cccd+\"@resident.local\", role:\"resident\", mustChangePassword:true, residentId}).

18. 10\. Trả 201 Created {message, resident}.

## 4.4 Sơ đồ Trình tự -- Đăng ký Tạm trú {#sơ-đồ-trình-tự-đăng-ký-tạm-trú}

19. 1\. Client → POST /api/residents/:id/tam-tru {address, start, end, reason, phone}.

20. 2\. requireAuth + requireRole(\"admin\",\"staff\").

21. 3\. Resident.findById(id) → không tìm thấy → 404.

22. 4\. Thiếu address/start/end/phone → 400.

23. 5\. end \< start → 400 \"Ngày kết thúc không được nhỏ hơn ngày bắt đầu\".

24. 6\. status == \"Tạm vắng\" → 409 \"Đang tạm vắng, không thể đăng ký tạm trú\".

25. 7\. resident.status = \"Tạm trú\"; resident.tamTru = {address, start, end, reason, phone}.

26. 8\. addHistory(resident, \"Đăng ký tạm trú\", req.user.name) → push vào history\[\].

27. 9\. resident.save() → lưu MongoDB Atlas.

28. 10\. Trả 200 {message, resident}.

## 4.5 Frontend -- Cấu trúc màn hình {#frontend-cấu-trúc-màn-hình}

|  |  |  |  |
|----|----|----|----|
| **Route** | **Component** | **Quyền** | **Chức năng** |
...
# CHƯƠNG 5. TRIỂN KHAI VÀ KIỂM THỬ (Tuần 7) {#chương-5.-triển-khai-và-kiểm-thử-tuần-7}

## 5.1 Thuật toán Business Rules {#thuật-toán-business-rules}

**BR-006 -- Không xóa cư dân đang lưu trú:**
...
**Thuật toán getResidentStats (Dashboard):**
...
## 5.2 Xử lý lỗi {#xử-lý-lỗi}

|  |  |
|----|----|
| **HTTP Code** | **Trường hợp trong source code** |
...
## 5.3 Kiểm thử {#kiểm-thử}

|  |  |  |
|----|----|----|
| **Loại** | **Công cụ** | **Kịch bản** |
...
## 5.4 Tiêu chuẩn Code & Kế hoạch {#tiêu-chuẩn-code-kế-hoạch}
